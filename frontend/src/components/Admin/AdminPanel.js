...
const handleDeleteClick = async (movie) => {
    ...
    new Promise((resolve) => {
      const ConfirmComponent = () => (
        <div>
          ...
          Are you sure you want to delete the movie <span className='text-rose-700'>
            "{movie.title}"?
            </span>
          </div>
          <div className='text-center mt-2'>
            <button 
              ...
              style={{ marginRight: '50%', backgroundColor:'skyblue', color:'white', padding:'3px',borderRadius:'30%',border:'2px solid black' }}
            >
              Yes
            </button>
            <button 
              ...
              style={{backgroundColor:'red',color:'white', padding:'3px', borderRadius:'30%',border:'2px solid black'}}
            >
              No
            </button>
          </div>
  ...
  return (
    <div
      ...
      className="admin-panel grid grid-cols-6"
    >
        ...
        <ul className="grid md:grid-cols-3 sm:grid-cols-2 lg:grid-cols-5 xl:grid-cols-6 gap-3">
          {movies.map((movie) => (
            <li
              ...
              key={movie._id}
            >
              <div className="h-[80px] my-3 font-extrabold font-mono text-lg">
                {movie.title}
              </div>
              <img
